...
const { downloadAudio, downloadVideo, mediaToSticker, cropMedia, stickerToPhoto, stickerToVideo } = require('../raymond/player/media');

module.exports = { 
  name: 'search', 
  description: 'Handle search-based media commands like play, sticker, google, crop, etc.',
    ...
    try {
      switch (command) {
        case 'play': {
          ...
          fs.unlinkSync(filePath); // Ensure cleanup
          break;
        }
      ...
      await sock.sendMessage(from, { text: '❌ Something went wrong, please try again later.' }, { quoted: m });
    }
  }
};
